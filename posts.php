...
if (($levelRow['board_view'] > 2) && ($levelRow['board_view'] > $_SESSION['user_level'])){
...
} else { //if the user DOES meet the required permissions OR the required permissions are not greater than level 2,
		 //the following content is displayed

   if($_SESSION['signed_in']){ //only displays add new reply if signed in
	   echo '<div id="board_list_title"><span style="font-size:16pt;">' . strip_tags($topicNameRow['topic_subject']) . '</span><br /><a href="topics.php?bid=' . $bid . '">Topic List</a>
	   | <a href="reply.php?bid=' . $bid . '&tid=' . $tid . '">Add New Reply</a></div>';
   } else { //if the user is not signed in, they do not receive a prompt to add a reply. instead, they only
			//receive a prompt to return to the topic list
      echo '<div id="board_list_title"><span style="font-size:16pt;">' . strip_tags($topicNameRow['topic_subject']) . '</span><br /><a href="topics.php?bid=' . $bid . '">Topic List</a></div>';
   }
	
	//this query obtains all the posts in the topic
   $sql = "SELECT post_id, post_content, post_date, post_topic, post_by, user_name
	   FROM posts LEFT JOIN users ON posts.post_by = users.user_id WHERE post_board = '" . $bid . "' AND post_topic = '" . $tid . "'";
   $result = mysql_query($sql);

   if(!$result){ //if the query fails, this error message is generated
   ...
   } else { //otherwise, the topic is real so posts are displayed
   ...
   if ($_SESSION['user_level'] >= 3){ //this assumes that a moderator has a level of 3 or greater
